...
# BaseTool import for CrewAI v0.1.7
...
class BrowserBaseTool(BaseTool):
    ...
    def _run(self, clinic_name: str, clinic_url: str, agent_id: str) -> Dict[str, Any]:
        ...
        session = None
        try:
            print(f"\n🔍 Starting scrape for: {clinic_name}")
            print(f"   URL: {clinic_url}")
            
            # Create a new browser session with project_id
            ...
            print("   Scanning for appointment information...")
            ...
            print("✅ Successfully scraped appointment information")
            return result
            
        except Exception as e:
            error_msg = f"Error scraping {clinic_name}: {str(e)}"
            print(f"❌ {error_msg}")
            return {
                "success": False,
                "clinic_name": clinic_name,
            ...
            if session and hasattr(session, 'id'):
                print(f"   Cleaning up session {session.id}...")
                # Note: BrowserBase sessions will eventually time out on their own
                # if we can't close them programmatically
                try:
                    # Try different possible cleanup methods
                    if hasattr(self.browserbase_client.sessions, 'close'):
                        self.browserbase_client.sessions.close(session.id)
                    elif hasattr(self.browserbase_client.sessions, 'delete'):
                        self.browserbase_client.sessions.delete(session.id)
                    print("   Session cleanup completed")
                except Exception as e:
                    print(f"⚠️  Warning: Could not properly close session: {e}")
                    print("   The session will time out automatically after a period of inactivity")
                    print(f"   You can close it manually at: https://browserbase.com/projects/{os.getenv('BROWSERBASE_PROJECT_ID')}/sessions")
